...
def train_fit(subject: str, tested_on: Union[str, None], model_name: str, models_dic: Dict,
    ...
    plot_single_output(y_true, y_pred, out_labels, predictions_pdf)
...
def get_files(subject: str, is_general_model=False) -> List[str]:
    ...
    if is_general_model:
        parent_folder = f"E:\Spring25\Capstone\\re_ver\Result\\minh_kun_only_raw_200e"
    else:
        parent_folder = f"E:\Spring25\Capstone\\re_ver\Result\\minh_kun_only_raw_200e"
    # Get the model and plots folder
    model_file = f"{parent_folder}\\S{subject}_model.weights.h5"
    print(f"LOad at Model file: {model_file}")
...
def compile_and_train(window_object: WindowGenerator, model_name: str,
    ...
    model.load_weights(model_file)
    return model
def inference( subject, model_name: str, models_dic: Dict, input_width=20, shift=1,
              label_width=1,lr=0.001, features: List[str] = ['RMS'],
              sensors: List[str] = ['sensor 1'], out_labels: List[str] = [f"ankle moment"],
              batch_size=8) -> Tuple[np.array, np.array]:
    """
    This function will be used to make predictions on the last 20 rows of the dataset
    """
    model_file, _ , _ = get_files(subject, is_general_model=False)
    ...
    window_object = partial_window_generator_func(subject)
    test_set = window_object.inference_set
    #eval_set = window_object.evaluation_set
    #print(f"[DEBUG] Type comparison:\n - inference_set: \n - evaluation_set: {type(eval_set)}")
    
    

    #print("Inference window shape:", list(test_set.as_numpy_iterator()))
    model = models_dic[model_name](window_object)
    # compile the model using NADAM compiler and a custom Loss function
    model.compile(optimizer=keras.optimizers.Nadam(learning_rate=lr),
                  loss=keras.losses.MeanSquaredError())
...
def inverse_scale(y: np.ndarray, dataHandler: DataHandler):
    # Inverse-transform the angle (just 1 column)
    return dataHandler.angle_scaler.inverse_transform(y)
